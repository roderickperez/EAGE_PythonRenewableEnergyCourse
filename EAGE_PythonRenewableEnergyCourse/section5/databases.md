...
A synthetic example table (not reported national statistics) could look like this:
...
**Disadvantages:**
- Parent-child structures are rigid and many-to-many relationships are awkward
...
For an energy example, `generation.country_id` can reference `country.country_id`, while `generation.source_id` references `energy_source.source_id`. A foreign key must reference an existing primary or unique key in the parent table.
...
**Examples:** Amazon RDS or Aurora, Google Cloud SQL, Azure SQL Database, and Supabase Postgres. AWS, Google Cloud, and Azure are platforms rather than individual database products.
...
The **Data Storage Service** is a component of the cloud platform where energy-related data can be retained. AWS S3, Google Cloud Storage, and Azure Blob Storage are object-storage services, not databases. They are useful for files and data lakes; queryable operational records normally use a database service, and analytical systems may use a warehouse or lakehouse.
...
- A single server can become a performance bottleneck and a single point of failure unless replication and failover are configured
...
NoSQL is an umbrella term covering document, key-value, wide-column, and graph databases. Many store structured or semi-structured records and some support horizontal scaling; capabilities and consistency guarantees vary by product.
...
* **NoSQL Database**
Energy data can be stored in a document database such as MongoDB when records have variable, semi-structured fields. The database still has an application-level schema, indexes, and validation rules; “schema flexible” does not mean that structure or governance is unnecessary.
...
| Managed cloud database   | Managed backups, scaling, and availability | Cost, vendor constraints, network dependency | Amazon RDS, Google Cloud SQL |
